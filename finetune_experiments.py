...
def build_core_model(base_model_name, fc_size, num_classes, optimizer=None):
  ...
  #base = base_model(input_tensor=input_t, weights='imagenet', include_top=False)
  ...
  #gad = GlobalAveragePooling2D()
  #d1 = Dense(fc_size, activation="relu")
  #d2 = Dense(num_classes, activation="softmax")
  
  #model = tf.keras.Sequential([base, gad, d1, d2]) 
  

  # make whole model trainable for now 
  # freeze all base layers
  #for layer in base.layers:
  #  layer.trainable = False
  return model 

def load_pretrained_model(base_model_name, fc_size, num_classes, optimizer=None):
  model = build_core_model(base_model_name, fc_size, num_classes)
  if not optimizer:
    optimizer = 'rmsprop'
...
def build_parallel_gpu_model(gpus, init_model, fc_size, num_classes, config, args):
  """ Build a parallel model over several gpus """
  with tf.device('/cpu:0'):
    model = build_core_model(init_model, fc_size, num_classes, args.optimizer)
  
  # log model here before we parallelize it
  log_distrib_model_params(model, config, args)
...
def build_single_gpu_model(init_model, fc_size, num_classes, args):
  """ Build a regular model """
  model = load_pretrained_model(init_model, fc_size, num_classes, args.optimizer)
  print(model.summary())
  return model

def log_distrib_model_params(model, wandb_config, args):
  """ Extract params of interest about the model (e.g. number of different layer types).
      Log these and any experiment-level settings to wandb """
...
# try finetuning a big model--or maybe even training?
def test_large_model(args):
  ...
  if args.distrib == 1:
    ...
    model = build_single_gpu_model(args.initial_model, args.fc_size, args.num_classes, args)
    log_distrib_model_params(model, wandb.config, args)
  else:
    model = build_parallel_gpu_model(args.distrib, args.initial_model, args.fc_size, args.num_classes, wandb.config, args)
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    ...
    type=int,
    default=0,
    help="num gpus to parallelize across")
  parser.add_argument(
    "--optimizer",
    type=str,
    default="adam",
